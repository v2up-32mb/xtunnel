...
x-tunnel 多通道 WebSocket 隧道的 **Go 核心库（模块 `github.com/v2up-32mb/xtunnel`）**，
**客户端与服务端能力一体**——统一日志钩子（`SetLogf`/`LogEvent`）、共享协议与配置：

- **客户端能力**：多通道连接池、代理拨号适配、反向出网、中继、HotPair 预热（ECH/DoH 由 `xshared` 提供）
- **服务端能力**：隧道服务、连接池与背压、热表、反向监听（SOCKS5/HTTP 服务器由 `xshared` 提供）
...
- **代理拨号适配**（`proxy_dialer.go`）：池 → `xshared/dialer` 接口的实现
  - TCP 流拨号（注册 → MsgTCPConnect（Hot Pair/广播）→ 等待建立 → 内存管道）
  - UDP 通道拨号（UDP ASSOCIATE 语义、端口黑名单、IPStrategy 过滤、懒启动竞速）
...
- **能力分层**：通用能力（ECH/DoH 用 [`xshared`](https://github.com/v2up-32mb/xshared) 的 `ech` 便捷工厂、
  SOCKS5/HTTP 服务器与解析工具、内存管道 `pipe`）全部由 `xshared` 提供，本库只保留核心协议。
...
| 版本 | 协议 | 服务端需跟随 | 下游代码改动 |
|---|---|---|---|
| v0.3.1 | +`MsgHotPairBegin`(0x23) | 否（可选,新端先发旧端忽略） | 无破坏性；壳可选择性消费 `ev.Domain` 结构化事件 |
...
- 除 v0.2.0 新增 `MsgHotPairNotify`(0x22, 需服务端跟随)外，消息集自 v0.2.0 起仅**加法式**
  新增 `MsgHotPairBegin`(0x23, v0.3.1, 向后兼容)；其余版本均为客户端行为/展示/日志改动。
