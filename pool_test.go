...
func TestWriteWorkerAggregatedJobsReleaseAllQueueBytes(t *testing.T) {
	...
	msg1 := protocol.EncodeMessage(protocol.MsgTCPData, "cid", nil, []byte("hello"))
	msg2 := protocol.EncodeMessage(protocol.MsgTCPData, "cid", nil, []byte("world"))
...
func TestUnregisterUDPAssociationReturnsPromptly(t *testing.T) {
	...
	assoc := &poolUDPChannel{
		pool:   p,
		connID: "udp-conn",
		down:   make(chan udpDatagram, 16),
		done:   make(chan struct{}),
	}
...
func TestClientPoolStartReturnsPromptlyWithLazyECH(t *testing.T) {
	cfg := DefaultConfig()
	...
	// 共享 ECH 语义：配置懒加载（首次拨号按需获取），Start 不应被 ECH 阻塞
	done := make(chan struct{})
	go func() {
		p.Start(nil)
		close(done)
	...
	case <-time.After(150 * time.Millisecond):
		t.Fatalf("Start() 在 ECH 懒加载语义下不应阻塞超过 150ms")
	}
