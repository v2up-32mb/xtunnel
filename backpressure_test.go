...
func TestBackpressureState(t *testing.T) {
	...
	pool.handleBackpressure(protocol.BackpressureSlowDown)
	if atomic.LoadInt32(&pool.backpressureState) != int32(protocol.BackpressureSlowDown) {
	...
	pool.handleBackpressure(protocol.BackpressurePause)
	...
	pool.handleBackpressure(protocol.BackpressureNormal)
	if atomic.LoadInt32(&pool.backpressureState) != int32(protocol.BackpressureNormal) {
...
func TestBackpressureDelay(t *testing.T) {
	...
	atomic.StoreInt32(&pool.backpressureState, int32(protocol.BackpressurePause))
	delay = pool.getBackpressureDelay()
	if delay != -1 {
